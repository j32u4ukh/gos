...
func RegisterCommand(rootCmd *cobra.Command) {
	taskCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			cors, err := cmd.Flags().GetInt32("cors")
			if err != nil {
				fmt.Printf("Failed to get string kind, err: %+v", err)
				return
			}
			useCors = cors == 1
			fmt.Printf("port: %d, kind: %s, useCors: %v\n", port, kind, useCors)
			switch kind {
			case "ans":
				RunAns(port)
			case "ask":
...
func RunAsk(ip string, port int32) {
	asker, err := gos.Bind(0, ip, int(port), define.Http, nil, nil, nil)
...
func DemoNativeHttpRequest(port int32) {
	client := &http.Client{}
...
func DemoNativeHttpRequest2(port int32) {
	client := &http.Client{}
...
func DemoNativeHttpServer(ip string, port int32) {
...
func DemoMethodHead(port int32) {
	// 创建一个路由器
