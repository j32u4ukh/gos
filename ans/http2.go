...
func (a *HttpAnser2) read() bool {
	...
	a.currContext = a.contexts[a.currConn.GetId()]

	// 讀取 第一行
	if a.currContext.State == 0 {
		if a.currConn.CheckReadable(a.currContext.HasLineData) {
			a.currConn.Read(&a.readBuffer, a.currContext.ReadLength)

			// 拆分第一行數據
			a.lineString = strings.TrimRight(string(a.readBuffer[:a.currContext.ReadLength]), "\r\n")
			fmt.Printf("(a *HttpAnser) Read | firstLine: %s\n", a.lineString)

			if a.currContext.Request2.ParseFirstLine(a.lineString) {
				if a.currContext.Request2.Method == ghttp.MethodGet {
					// 解析第一行數據中的請求路徑
					a.currContext.Request2.ParseQuery()
				}
				a.currContext.State = 1
	...
	// 讀取 Header 數據
	if a.currContext.State == 1 {
		var key, value string
		var ok bool

		for a.currConn.CheckReadable(a.currContext.HasLineData) && a.currContext.State == 1 {
			// 讀取一行數據
			a.currConn.Read(&a.readBuffer, a.currContext.ReadLength)
			...
			if ok {
				...
				if _, ok := a.currContext.Header[key]; !ok {
					a.currContext.Header[key] = []string{}
				}

				value = strings.TrimLeft(value, " \t")
				// value = strings.TrimRight(value, "\r\n")
				a.currContext.Header[key] = append(a.currContext.Header[key], value)
				...
				if contentLength, ok := a.currContext.Header["Content-Length"]; ok {
					...
					a.currContext.ReadLength = int32(length)
					a.currContext.State = 2
	...
	// 讀取 Body 數據
	if a.currContext.State == 2 {
		if a.currConn.CheckReadable(a.currContext.HasEnoughData) {
			// ==========
			// 讀取 data
			// ==========
			// 將傳入的數據，加入工作緩存中
			a.currConn.Read(&a.readBuffer, a.currContext.ReadLength)
			fmt.Printf("(a *HttpAnser) Read | %s\n", string(a.readBuffer[:a.currContext.ReadLength]))

			// 考慮分包問題，收到完整一包數據傳完才傳到應用層
			a.currWork.Index = a.currConn.GetId()
			a.currWork.RequestTime = time.Now().UTC()
			a.currWork.State = 1
			a.currWork.Body.AddRawData(a.readBuffer[:a.currContext.ReadLength])
			a.currWork.Body.ResetIndex()
...
func (a *HttpAnser2) SetWorkHandler() {
	a.workHandler = func(w *base.Work) {
		a.currContext = a.contexts[w.Index]

		if handler, ok := a.Handlers[a.currContext.Method]; ok {
			if functions, ok := handler[a.currContext.Query]; ok {
				for _, f := range functions {

					f(a.currContext)

					a.currContext.SetHeader("Connection", "close")

					// 將 Response 回傳數據轉換成 Work 傳遞的格式
					bs := a.currContext.ToResponseData()
					fmt.Printf("Response: %s\n", string(bs))
					w.Body.AddRawData(bs)
					w.Send()
				}
			} else {
				a.errorRequestHandler(w, a.currContext, "Unregistered http query.")
			}
		} else {
			a.errorRequestHandler(w, a.currContext, "Unregistered http method.")
		}
	}
}
...
func (a *HttpAnser2) shouldClose(err error) bool {
	...
	a.currContext = a.contexts[a.currConn.GetId()]
	if a.currContext.State == 4 && a.currConn.WritableLength == 0 {
		fmt.Printf("(a *HttpAnser) shouldClose | Conn(%d) 完成數據寫出，準備關閉連線\n", a.currConn.GetId())
		a.currContext.State = 0
		return true
	}
	return false
}
